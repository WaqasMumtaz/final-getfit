...
class ChatInbox extends React.Component {
    ...
    chatInboxFunction = async () => {
        ...
        // console.log('yesterday function >>', yesterdayDate)
        this.setState({
    ...
    getUsersLastMgs = async (userNamesData, currentUserId) => {
       ...
       await db.ref('chatRoom').on("value", snapshot => {
            ...
            for (var i = 0; i < userNamesData.length; i++) {
                ...
                // console.log('userMessageAndNameData >>',userMessageAndNameData)
            ...
            this.setState({
                ...
                // console.log('State Array >>', this.state.messageUser)
    ...
    sendOppentUserData(userData, type) {
        //  console.log('userData >>', userData, 'Type read >>', type);
        ...
        db.ref('chatRoom').on("value", snapshot => {
            ...
            for (var j in data) {
                let firbaseData = data[j];
                if (firbaseData.reciverId == this.state.currentUserId && firbaseData.senderId == userData.userId) {
                    if (firbaseData.status == 'sent' || firbaseData.status == 'delivered') {
                        //  console.log(firbaseData ,'sent status messages opponent' , j , 'iteration')
    ...
    render() {
        ...
        // console.log('yesterdayDate >>', yesterdayDate.slice(0, 2));
        // console.log('messageUser >>', messageUser);
        const uniq = new Set(messageUser && messageUser.map(e => JSON.stringify(e)));
        // console.log('Uniq >>', uniq)
        const uniqUsers = Array.from(uniq).map(e => JSON.parse(e));
        //   console.log('Res usermessages >>', uniqUsers);
        ...
        let showDate;
        let showDay;
        const senderName = uniqUsers && uniqUsers.map((elem, key) => {
            ...
            if (elem.date != undefined) {
                ...
                else if (Number(elem.date.slice(3, 5)) != Number(yesterdayDate.slice(3, 5))
                    && Number(elem.date.slice(3, 5)) != Number(todayDate.slice(3, 5))
                    && Number(elem.date.slice(0, 2)) == Number(yesterdayDate.slice(0, 2))
                    && Number(elem.date.slice(6)) == Number(yesterdayDate.slice(6))) {
                    const date = elem.date.slice(3, 5);
                    const month = elem.date.slice(0, 2);
                    const year = elem.date.slice(6);
                    const fullDate = year + '-' + month + '-' + date;
                    // console.log('Full DAte >>',fullDate );
                    const momenDate = moment(fullDate, "YYYY-MM-DD");
                    const dow = momenDate.format('dddd');
                    //  console.log('Day Name >>>', dow);
                    showDate = dow;
            ...
            return (
                <TouchableOpacity style={styles.mainInboxContainer}
                    onPress={this.sendOppentUserData.bind(this, elem, 'seen')}
                    key={key}
                >
                    ...
                    {elem.date != undefined && elem.time != undefined && elem.message != undefined && elem.counter != undefined
                        && elem.msgStatus != undefined && elem.type != undefined ?
                        ...
                        <View style={styles.checkAndMessgContainer}>
                                                    ...
                                                    elem.type == 'mp3' || elem.type == 'wma' ? <View style={styles.imagesAndText}>
                                                        ...
                                                        elem.type == 'mp4' ? <View style={styles.imagesAndText}>
                                                            ...
                                                            elem.type == 'weeklyReport' ? <View style={styles.imagesAndText}>
                                                                ...
                                                                : elem.type == 'text' ?
                                                                    <View style={{ flexDirection: 'row', }}><Text style={{ width: 100, color: 'gray', fontSize: 12, marginRight: '11%' }}
                                                                        numberOfLines={1}>{elem.message}</Text></View>
                                                                    :
                                                                    null
                            }
                            {
                                elem.time != undefined ?
                                    <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
                                        <Text style={{ color: 'gray', fontSize: 12, marginRight: 8 }}>{elem.time}</Text>
                                        {showDate != '' && showDate == 'Today' || showDate == 'Yesterday' ? <Text style={{ color: 'gray', fontSize: 12, marginRight: 8 }}>{showDate}</Text>

                                            :
                                            <Text style={{ color: 'gray', fontSize: 12, marginRight: 8 }}>{showDate}</Text>
                            ...
                            <View><Text style={{ color: 'blue', fontSize: 14, marginRight: 13, marginBottom: 5 }}>{elem.counter >= 1 ? elem.counter : null}</Text></View>
